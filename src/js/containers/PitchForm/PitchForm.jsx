...
class PitchForm extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      this.props.pitchform.projectSubmitted !=
      nextProps.pitchform.projectSubmitted
    ) {
  ...
  onSubmit(values) {
    ...
    if (this.state.totalSize > 250) {
      this.setState({
        fileMsg: 'Your file(s) is over 250 MBs, Est. time of 5-10 minutes'
      });
    }
    if (this.state.totalSize < 250) {
      this.setState({
    ...
    if (fileLength > 0) {
      files.forEach(file => {
        ...
        this.fileUpload(file).then(response => {
          ...
          this.setState(
            {
              filesUploaded: [...this.state.filesUploaded, fileLocation],
              timeEstimate: ''
            },
            () => {
  ...
  onChange(e) {
    ...
    this.setState({
      totalSize: this.state.totalSize + e.target.files[0].size / 1000000
    });
                    ...
                    <div className='jumbotron jumbotron-fluid'>
                      <div className='container'>
                          ...
                          <div className='card-body text-center '>
                            ...
                            <ul className='list-group list-group-flush'>
                              <li className='list-group-item'>
                                {' '}
                                Video(s) can only be a maximum of 5 minutes or
                                500 Mb.{' '}
                              </li>
                              <li className='list-group-item'>
                                {' '}
                              ...
                              <li className='list-group-item'>
                                {' '}
                                Video(s) will not be uploaded till you hit
                                submit on final page.
                              </li>
                              <li className='list-group-item'>
                                {' '}
                                Video(s) must have correct file type (.mp4,
                                .m4v, .mov).{' '}
                              </li>
                              <li className='list-group-item'>
                                {' '}
                                To upload multiple videos just click choose file
                                after you see your first video in staged
                                uploads.
                              </li>
                            </ul>
                          </div>
                        ...
                        <div className='p-3 text-center'>
                          <h4 className='text-center'>
                            {' '}
                            Upload A file (.pptx, .doc, .docx, .pdf){' '}
                          </h4>
                          <div className='card p-3'>
                            <div className='card-body text-center '>
                              <div className='text-center '>
                              ...
                              <h5 className='pt-3'>
                                {' '}
                                Requirments for Documents{' '}
                              </h5>
                              <ul className='list-group list-group-flush'>
                                <li className='list-group-item'>
                                  {' '}
                                  Total amount of size of documents should be
                                  maximum of 500Mb (Including Video){' '}
                                </li>
                                <li className='list-group-item'>
                                  {' '}
                                  Should not contain any inappropriate content.{' '}
                                ...
                                <li className='list-group-item'>
                                  {' '}
                                  Document(s) will not be uploaded till you hit
                                  submit on final page.
                                </li>
                                <li className='list-group-item'>
                                  {' '}
                                  Document(s) must have correct file type (.doc,
                                  .docx, .pptx, .pdf).{' '}
                                </li>
                                <li className='list-group-item'>
                                  {' '}
                              ...
                              <ul className='list-group list-group-flush'>
                                {this.state.files.map(f => (
                                  <li className='list-group-item' key={f.name}>
                                    {f.name} - {this.convertToMega(f.size)}{' '}
                                    MegaBytes
                                  </li>
                                ))}
                              </ul>
